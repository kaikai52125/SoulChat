...
> V0.0.4 核心特性：给一句话研究主题，SoulChat 自主完成「规划 → 多源检索 → 提炼 → 撰写 → 汇总」，产出结构化、带可点来源的 Markdown 报告。引擎是**纯异步生成器**，与传输层解耦——在线发起经 Redis bus 流式 + 断线续传，定时任务直接复用同一引擎。v1 跑通后做了 v2 质量重构（对标 GPT Researcher / STORM）+ 来源质量过滤 + 研究指令润色。本篇只讲后端。
